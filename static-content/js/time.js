...
let displayRedcords = (records) => {
  ...
  records.forEach(record => {
    ...
    span.textContent = convertTimeFromSec(record.seconds);
...
// XXXsからXXhXXmXXsに変換する
let convertTimeFromSec = (totalSec) => {
  sec = totalSec;
...
let displayTotalNum = (totalNum) => {
  ...
  totalRecords.textContent = "Total: " + convertTimeFromSec(totalNum);
}

// 表示する関数
let display = (data) => {
  displayRedcords(data.repositories);
  displayTotalNum(data.total_seconds);
}
