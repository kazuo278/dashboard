...
let formatStatus = (type) => {
  ...
  } else if (type === "REUSABLE_WORKFLOW") {
    span.textContent = "ReusableWorkflow";
  } else {
    span.textContent = "不明";
  }
  return span;
...
let createRequestUri = () => {
  var uri = window.location.protocol + "//" + window.location.host + "/details";
  ...
  var run_attempt = document.getElementById("run_attempt").value;
  if (run_attempt) {
    params.append("run_attempt", run_attempt);
  }
  ...
  var using_path = document.getElementById("using_path").value;
  if (using_path) {
    params.append("using_path", using_path);
  }

  var using_ref = document.getElementById("using_ref").value;
  if (using_ref) {
    params.append("using_ref", using_ref);
  }
...
let displayRedcords = (records) => {
  ...
  records.forEach(record => {
    ...
    tdJobId.classList.add("d-none");
    tdJobId.textContent = record.job_id;
...
let initDashboard = () => {
  ...
  runAttempt = params.get("run_attempt");
  if (jobId == null && runId == null && runAttempt == null) {
    document.getElementById("started_at").value = getToday();
  } else {
    document.getElementById("job_id").value = jobId
    document.getElementById("run_id").value = runId
    document.getElementById("run_attempt").value = runAttempt
  }
  // イベントリスナー登録
  // 検索ボタン押下時に検索実行
